...
class PINN(nn.Module):
    def __init__(self, n_hidden=20, n_layers=1):
        ...
        # Trainable smooth part constant
        self.A = nn.Parameter(torch.tensor([0.0]))

        # Trainable boundary layer constants
        self.B = nn.Parameter(torch.tensor([0.0]))
        self.C = nn.Parameter(torch.tensor([0.0]))

    def forward(self, x, eps):
        
        u_s = self.A * f(x) / c(x)   # The smooth part of solution u
        left_bl = self.B * torch.exp(-x / eps)         # Left boundary layer
        right_bl = self.C * torch.exp(-(1 - x) / eps)  # Right boundary layer
        u_r = self.net(x)           # The remainder of solution u

        u = u_s + left_bl + right_bl + u_r
