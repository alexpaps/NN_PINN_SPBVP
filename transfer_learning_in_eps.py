...
# -----------------------------
torch.manual_seed(0)
model_eps1 = PINN(n_hidden=n_hidden, n_layers=n_layers)
model_eps1 = train(model_eps1, eps=eps_transfer, Nx=Nx, epochs=epochs, lr=lr)
torch.save(model_eps1.state_dict(), "pinn_eps1.pth")


# -----------------------------
# Step 2: Train for ε = 0.01 using ε=1 weights
# -----------------------------
model_eps = PINN(n_hidden=n_hidden, n_layers=n_layers)
model_eps.load_state_dict(torch.load("pinn_eps1.pth"))
