...
# -----------------------------
def train(model, eps, Nx, epochs, lr, alpha=0.0, beta=0.0, gamma=10, zeta=10):
    ...
    for epoch in range(epochs):
        ...
        bc_loss = gamma*(u_bc[0] - alpha)**2 + zeta*(u_bc[1] - beta)**2
...
model_eps = train(model_eps, eps=eps, Nx=Nx, epochs=epochs, lr=lr, gamma=gamma, zeta=zeta)
...
plt.suptitle(
    f"-ε²u''(x) + u(x) = 1, u(0)=u(1)=0 | ε={eps} | Epochs: {epochs} | Nodes: {n_hidden} | Layers: {n_layers} | Samples: {Nx} | LR: {lr} | γ: {gamma} | ζ: {zeta}",
    fontsize=11,
    fontweight='bold'
)
