...
# Training function
# -----------------------------
def train(model, eps, Nx, epochs, lr):
...
# --- PINN solution ---
u_eps = x_plot * (1 - x_plot) * model_eps(x_plot)
...
plt.suptitle(f"-ε²u''(x) + u(x) = 1, u(0)=u(1)=0 | ε={eps} | Epochs: {epochs} | Nodes: {n_hidden} | Layers: {n_layers} | Samples: {Nx} | LR: {lr}", 
